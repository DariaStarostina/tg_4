...
async def cmd_start(message: types.Message): 
    ...
    await message.reply("Салам! Как твое имя, дружок?") 
...
async def process_name(message: types.Message, state: FSMContext): 
    ...
    await Form.next() 
    await message.reply("Какая фамилия у тебя?") 
...
async def process_gender_invalid(message: types.Message): 
    return await message.reply("Ответ неверный. Выберите нужный вариант на клавитуре.") 
...
async def process_gender(message: types.Message, state: FSMContext): 
    async with state.proxy() as data: 
        ...
        await bot.send_message( 
            message.chat.id, 
            md.text( 
                md.text('Рада познакомиться!,', md.bold(data['firstname'] + " " + data['lastname'])), 
